    ...
    const SECTIONS = [
        ...
        },
        {
            ...
            how: [
                ...
                '<strong>Right-clicking any selected tile</strong> works on the whole selection too: <strong>Set Material</strong> reads <em>“— N tiles…”</em> and assigns to all of them. Right-clicking a tile <em>outside</em> the selection drops back to that one tile.',
        ...
        },
        {
            ...
            what: 'The boundary a transition set draws is a clean curve, which reads as generated. The <strong>🌿 Organic edge</strong> panel breaks it up into a ragged, flecked, hand-painted one — in the <strong>Full Set</strong> tab of Make Transition, and in <strong>Make Wang Set</strong> too. It is a different thing from <strong>Make Organic Transition</strong> below: that scatters the overlay into loose patches across a single tile, this reshapes the boundary of a whole set while keeping every tile joined to its neighbours.',
            how: [
...
'Pick an <strong>Edge style</strong>. <strong>Blobs</strong> gives soft rounded lobes, <strong>Spikes</strong> short tapered teeth, <strong>Drips</strong> long reaching fingers, <strong>Clumps</strong> chunky lobes with gaps, <strong>Fray</strong> a fine ragged fringe. Each one loads its own suggested settings and a line saying what it suits.',
            ...
            tip: 'On a <strong>Wang set</strong> the panel offers the same styles minus <strong>Drift</strong>, because Drift is the one control that reaches the tile border and a Wang set\u2019s whole job is that any tile can sit beside any other compatible one. A Wang set is also reused far more heavily across a level, so keep <strong>Amount</strong> lower there. An organic edge makes <em>repetition</em> more obvious, not less. A straight boundary tiles invisibly; a distinctive blob does not, so a long run of one organic tile reads as a repeating motif. That is what <strong>Alternates</strong> is for. Keep the sliders modest for edges you will lay in long runs, and save the strong settings for a boundary that appears once or twice.'
        },
        {
        ...
        },
        {
            ...
            how: [
                '<strong>Right-click</strong> a tile → <strong>Generate Variations</strong>.',
